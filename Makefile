...
.PHONY: help build build-no-cache run run-script shell clean demo demo-ui run-pyqt run-tkinter
...
help:
	...
	@echo "  make run-pyqt               Run demo script in Docker with PyQt via Xpra (open http://localhost:14500)"
	@echo "  make run-tkinter            Run demo script in Docker with Tkinter via Xpra (open http://localhost:14500)"
